-- Migration: 0011_fix_upsert_profile_email
-- Description:
--   The upsert_staff_profile function always overwrote the email column on
--   conflict, even when called from save-step with p_email = ''. This would
--   blank out the email stored from signup. Fix: on conflict, coalesce
--   p_email with the existing value so a blank p_email is a no-op.
...
begin
  ...
  on conflict (id) do update set
    -- Preserve existing email if caller passes empty string
