...
export async function POST(request: NextRequest) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          error: {
            code: 'INVALID_STATE',
            message: `Cannot reject: current status is ${staff.verification_status}`,
            requestId,
          },
        },
        { status: 400 }
    ...
    if (updateError) {
      console.error('[POST /api/admin/staff/reject] update failed', { requestId, error: updateError });
      return NextResponse.json(
        { ok: false, error: { code: 'UPDATE_FAILED', message: 'Failed to reject staff', requestId } },
        { status: 500 }
      );
    }
    ...
    if (auditError) {
      console.error('[POST /api/admin/staff/reject] audit log failed', { requestId, error: auditError });
    }
    ...
    console.error('[POST /api/admin/staff/reject]', { requestId, error });
    return NextResponse.json(
